...
setInterval(async () => {
  ...
  if (versionCheckInterval <= 0) {
    var getLatestCommit = await axios.get("https://api.github.com/repos/msamods/hishana/commits")
...
async function Hishana() {
  ...
  sock.ev.on("messages.upsert", async (msg) => {
    try {
      ...
      console.log(error);
      await sock.sendMessage(sock.user.id, { text: `*⚠️ Hishana Error:*\n${error}` });
    }
  });
